...
const CalendarDrawer = ({ toggleDrawer, onSubmit, clickedDay, open,  handleCancel, dayName, control, handleSubmit, isMobile }) => {
  ...
  return (
    <Drawer
      ...
      open={open}
      onClose={toggleDrawer(false, 'empty')}
      anchor={'right'}
    >
      <Box>
        <Grid2 container>
          <Grid2 item size={6}>
            <Typography variant={isMobile? "h5" : "h3"} sx={{ pb: isMobile? 2 : 1, mt: 8, fontSize: isMobile? '2rem': '2.5rem'}}>
              {isMobile ? dayName.substring(0, 3) : dayName}
            </Typography>
          </Grid2>
          <Grid2 item size={6}>
            <Typography variant={isMobile? "h4": "h1"} sx={{ pb: isMobile? 0: 1, justifySelf: 'flex-end', fontSize: isMobile? '3rem': '4.5rem' }}>
              {  clickedDay  || "" }
            </Typography>
          </Grid2>
        </Grid2>
