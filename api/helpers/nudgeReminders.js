...
async function getNudgeReminders(datetime, timezone) {
  ...
  else if ((x.date() > y.date()) && (diffHours > 8 && reminderHour > 18)) {
...
async function nudgeReminderContent(name, intention, dateScheduled, datetime, timezone) {
  ...
  if ((timezone == 'America/Honolulu' && nudgeReminders.length == 1) || ( dayjs(datetime).date() > dayjs().date() && nudgeReminders.length === 1 )) {
    message = `Good Evening ${name}! You have scheduled a reminder for ${dateScheduled}.  Your intention is to focus on ${intention}.`;
     ...
     message = `Hello ${name}! This is just a quick reminder that you have scheduled a leave that takes place today at ${reminderTimeStr} to focus on ${intention}.`;
